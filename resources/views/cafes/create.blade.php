...
<div class="min-h-screen bg-[#F5EFE0] py-8">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex items-center gap-1.5 text-sm text-[#A08060] mb-5">
            ...
            <span class="text-[#B5642A]">Novo Café</span>
        </nav>

        <div class="bg-[#FDF9F3] rounded-xl border border-[#E2D4B8] overflow-hidden">
            <div class="px-6 py-5 border-b border-[#EDE5D2]">
                <h1 class="text-lg font-semibold text-[#1C1008]" style="font-family: Georgia, serif;">Adicionar Café</h1>
                <p class="text-xs text-[#A08060] mt-1">Preencha os dados do novo produto</p>
            </div>

            <div class="px-6 py-6">
                <form action="{{ route('cafes.store') }}" method="POST" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            ...
                            <select name="categoria_id" id="categoria_id"
                                    class="w-full px-3 py-2.5 rounded-lg border text-sm text-[#1C1008] bg-[#FDF9F3]
                                ...
                                @foreach(['clara' => ['Clara','#F5E8C0','#D4C090'], 'media' => ['Média','#C4893A','#A06828'], 'escura' => ['Escura','#2E1A0E','#1C1008']] as $val => [$label, $dotBg, $dotBorder])
                                    <label class="cursor-pointer">
                                        ...
                                        <div class="flex flex-col items-center gap-1.5 p-3 rounded-lg border border-[#D4C4A0] bg-[#FDF9F3]
                                            ...
                                            <span class="text-xs font-medium text-[#7A5A3A] peer-checked:text-[#B5642A]">{{ $label }}</span>
                                        </div>
                                    </label>
                                @endforeach
                            </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="flex">
                                <input type="number" name="estoque_kg" id="estoque_kg"
                                        step="0.01" min="0"
                                        value="{{ old('estoque_kg') }}"
                                        placeholder="0,00"
                                        class="flex-1 px-3 py-2.5 rounded-l-lg border text-sm text-[#1C1008] bg-[#FDF9F3] placeholder-[#C0A880]
                                                focus:outline-none focus:ring-2 focus:ring-[#B5642A]/30 focus:border-[#B5642A] transition-all
